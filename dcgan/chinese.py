...
url="./dataset/fonts/"
DroidSansFallbackFull = ImageFont.truetype(url+"DroidSansFallbackFull.ttf", font_size, 0)
fangzheng_fangsong = ImageFont.truetype(url+"fangzheng_fangsong.ttf", font_size, 0)
fangzheng_heiti = ImageFont.truetype(url+"fangzheng_heiti.ttf", font_size, 0)
Lantinghei = ImageFont.truetype(url+"Lantinghei.ttc", font_size, 0)
mingliu = ImageFont.truetype(url+"mingliu.ttc", font_size, 0)
NotoSansHans_Black = ImageFont.truetype(url+"NotoSansHans-Black.otf", font_size, 0)
NotoSansHans_Bold = ImageFont.truetype(url+"NotoSansHans-Bold.otf", font_size, 0)
NotoSansHans_DemiLight = ImageFont.truetype(url+"NotoSansHans-DemiLight.otf", font_size, 0)
#NotoSansHans_Light = ImageFont.truetype(url+"NotoSansHans-Light.otf", font_size, 0)
NotoSansHans_Medium = ImageFont.truetype(url+"NotoSansHans-Medium.otf", font_size, 0)
#NotoSansHans_Regular = ImageFont.truetype("NotoSansHans_Regular.otf", font_size, 0)
'''
...
def block(image, mode, size):
    ...
    if mode == 1:  # random rescaling
        ...
        image = image[int(center[0] - size_xy // 2):int(center[0] + size_xy // 2),
                int(center[1] - size_xy // 2):int(center[1] + size_xy // 2)]
    return image
...
def generate_images(number=81, part_taken=0.9999, to_file=False, file=''):
    ...
    if not to_file:
        ...
        for i in range(number):
            char_id = np.random.randint(len(charset) * part_taken, len(charset))
            font_id = np.random.randint(0, len(fonts))
            cv2.imwrite(file + 'chinese_' + str(i) + '.png',
                        generate_image(char_id, font_id))


if __name__ == "__main__":
    generate_images(number=16 * 500, part_taken=0.014, to_file=True, file='F:\\Python project\\GANMNIST\\dataset\\chinese\\')